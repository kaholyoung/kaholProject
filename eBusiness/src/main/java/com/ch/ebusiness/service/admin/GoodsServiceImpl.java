...
public class GoodsServiceImpl extends ServiceImpl<GoodsRepository,Goods> implements GoodsService{
	...
	@Autowired
	private TypeRepository typeRepository1;
...
//	  	int totalCount = goodsRepository.selectAllGoods();
		...
		int totalCount = goodsRepository.selectList(null).size();
		//计算共多少页
	  	int pageSize = 5;
...
//			String path = "C:\\workspace-spring-tool-suite-4-4.1.1.RELEASE\\eBusiness\\src\\main\\resources\\static\\images";
			...
			String path = "../../resources/static/images";
			//获得上传文件原名
		...
		if("add".equals(act)) {
			...
			if(goodsRepository.insert(goods) > 0)//成功
				return "redirect:/goods/selectAllGoodsByPage?currentPage=1&act=select";
			//失败
			return "admin/addGoods";
		}else {//修改
			int i = goodsRepository.updateById(goods);
			if(i > 0)//成功
	...
	public String toAddGoods(Goods goods, Model model) {
		model.addAttribute("goodsType", typeRepository1.selectList(null));
		return "admin/addGoods";
	}
	...
	public String delete(Integer id) {
		if(goodsRepository.selectFocusGoods(id).size() > 0
